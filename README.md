...
**Game logos** are the second key's whole job. A card draws the game's own lettering over its
artwork, and the only source that has that for a PlayStation title is SteamGridDB, which is
keyed by name rather than by store id. It is optional: without the key, logos still come from
Steam's own CDN, which answers for any game linked to a Steam app and needs no key at all. A
game no source has a logo for simply shows none, and is asked about again a week later.
